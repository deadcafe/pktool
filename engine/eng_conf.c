         ...
         const char *val)
{
    ...
    if (node) {
        ...
        node->conf.name = node->name;

        if (val) {
            snprintf(node->val, sizeof(node->val), "%s", val);
            node->conf.val = node->val;
        } else {
            node->conf.val = NULL;
        }
                    ...
                    struct rte_ether_addr *addr)
{
    ...
    if (asc) {
            if (ether_aton_r(asc, (struct ether_addr *) addr))
            return 0;
    }
    return -1;
}
                        ...
                        const struct rte_ether_addr *addr)
{
    ...
    rte_ether_format_addr(mac_asc, sizeof(mac_asc), addr);
