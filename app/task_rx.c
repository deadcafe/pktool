...
ipv4_hash(uint32_t hash,
          ...
          unsigned offset)
{
    const struct rte_ipv4_hdr *ipv4_hd;
...
ipv6_hash(uint32_t hash,
          ...
          unsigned offset)
{
    const struct rte_ipv6_hdr *ipv6_hd;
...
tcp_hash(uint32_t hash,
         ...
         unsigned offset)
{
    const struct rte_tcp_hdr *tcp_hd;
...
udp_hash(uint32_t hash,
         ...
         unsigned offset)
{
    const struct rte_udp_hdr *udp_hd;
...
sctp_hash(uint32_t hash,
          ...
          unsigned offset)
{
    const struct rte_sctp_hdr *sctp_hd;
                 ...
                 struct rte_net_hdr_lens *hdr_lens)
{
    ...
    if (proto == RTE_BE16(RTE_ETHER_TYPE_IPV4)) {
        ...
        if (ip4h->fragment_offset &
            RTE_BE16(RTE_IPV4_HDR_OFFSET_MASK | RTE_IPV4_HDR_MF_FLAG)) {
    ...
    if ((ptype & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_UDP) {
        ...
        if (udp_hd->dst_port == RTE_BE16(GTPU_PORT)) {
            ...
            if (likely(gtp_hd)) {
                ...
                ptype |= ptype_tunnel_gtp(RTE_BE16(RTE_ETHER_TYPE_IPV4),
                                          m,
                                          offset,
                                          hdrs);
            }
