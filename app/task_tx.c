...
struct base_frame_s {
    struct rte_ether_hdr eth_hd;
    struct rte_ipv4_hdr ip_hd;
    struct rte_udp_hdr udp_hd;
    struct rte_flow_item_gtp gtp_hd;
    struct rte_ipv4_hdr ip_hd_inner;
    struct rte_udp_hdr udp_hd_inner;
           ...
           struct eng_task_s *task)
{
    ...
    rte_eth_random_addr(priv->base->eth_hd.d_addr.addr_bytes);
    rte_eth_random_addr(priv->base->eth_hd.s_addr.addr_bytes);
    priv->base->eth_hd.ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);
